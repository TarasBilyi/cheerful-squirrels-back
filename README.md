...
- CORS підключено без параметрів (`app.use(cors())`) — без `credentials: true` та явного origin запити з кредами (куками) із фронтенду на іншому порту/домені будуть відхилятись браузером.
